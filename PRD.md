...
| `doctor [--fix]` | Reports the resolution facts first: the running binary and which rule of the chain (`TASKKLING_BINARY` → up-tree `.taskkling/bin` → config `binary_path` → `PATH`) picks it, the workspace root and whether discovery walked up to it, the absolute `tasks_dir` and its active-task count, and the version — stating that the binary and the workspace resolve independently. Reports rather than errors when no workspace is in scope (being lost is when it gets run); an explicit `--root` that is not a workspace stays a usage error. *The integrity + logical-resolution scan with deterministic A/B fixes — and so `--fix` — remains a stub (post-PRD).* |
